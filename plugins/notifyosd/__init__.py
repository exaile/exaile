...
class ExaileNotifyOsd(object):
            ...
            if track:
                if self.icon == self.stopicon or self.icon == self.pauseicon:
                    if self.use_media_icons:
                        self.update_track_notify(
                            type, player.PLAYER, track, self.icon)
                        return
                self.update_track_notify(type, player.PLAYER, track)
    ...
    def exaile_ready(self, type=None, data1=None, data2=None):
        if self.exaile.gui.tray_icon:
            self.tray_connection = self.exaile.gui.tray_icon.connect(
                'query-tooltip', self.on_tooltip)
...
def enable(exaile):
    ...
    event.add_ui_callback(EXAILE_NOTIFYOSD.on_play,
                          'playback_track_start', player.PLAYER)
    event.add_ui_callback(EXAILE_NOTIFYOSD.on_pause,
                          'playback_player_pause', player.PLAYER)
    event.add_ui_callback(EXAILE_NOTIFYOSD.on_stop,
                          'playback_player_end', player.PLAYER)
    event.add_ui_callback(EXAILE_NOTIFYOSD.on_resume,
...
def disable(exaile):
    event.remove_callback(EXAILE_NOTIFYOSD.on_play,
                          'playback_track_start', player.PLAYER)
    event.remove_callback(EXAILE_NOTIFYOSD.on_pause,
                          'playback_player_pause', player.PLAYER)
    event.remove_callback(EXAILE_NOTIFYOSD.on_stop,
                          'playback_player_end', player.PLAYER)
    event.remove_callback(EXAILE_NOTIFYOSD.on_resume,
                          'playback_player_resume', player.PLAYER)
    ...
    if EXAILE_NOTIFYOSD.exaile.gui.tray_icon:
        EXAILE_NOTIFYOSD.exaile.gui.tray_icon.disconnect(
            EXAILE_NOTIFYOSD.tray_connection)
