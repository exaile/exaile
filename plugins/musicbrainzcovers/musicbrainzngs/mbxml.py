...
def parse_attributes(attributes, element):
    ...
    for attr in element.attrib:
        ...
        if a in attributes:
            ...
            _log.debug("in <%s>, uncaught attribute %s",
...
def parse_inner(inner_els, element):
    ...
    for sub in element:
        ...
        if t in inner_els.keys():
            ...
            _log.debug("in <%s>, not delegating <%s>",
                       fixtag(element.tag, NS_MAP)[0], t)
    return result
...
def parse_release(release):
    ...
    elements = ["title", "status", "disambiguation", "quality",
                "country", "barcode", "date", "packaging", "asin"]
    ...
    if "artist-credit" in result:
        result[
            "artist-credit-phrase"] = make_artist_credit(result["artist-credit"])
...
def make_echoprint_request(echoprints):
    ...
    for recording, echoprint_list in echoprints.items():
        ...
        l = echoprint_list if isinstance(echoprint_list, list) else [
            echoprint_list]
        for e in l:
