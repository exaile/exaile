...
"""
    This plugin will display a GTK+ dialog with 'did you know..' information
    from Wikipedia when a song starts playing in Exaile
"""
...
# For HTTP requests. The APP variable doesn't exist yet, so we set this in
# initialize().
USER_AGENT = None
...
def initialize():
    ...
    ig=InfoGiver(APP,BUTTON)
    BUTTON.connect('clicked', ig.toggle_popups)

    APP.xml.get_widget('rating_toolbar').pack_start(BUTTON)
    BUTTON.show()

    #CONNS.connect(APP, 'play-track', play_track)
    CONNS.connect(APP.player, 'stop-track', stop_track)
    CONNS.connect(APP, 'track-information-updated',track_information_updated)
    return True
