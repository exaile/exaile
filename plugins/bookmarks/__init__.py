...
#        self.auto_db = {}
        self.exaile = exaile
        self.use_covers = settings.get_option(
            'plugin/bookmarks/use_covers', False)
        self.counter = 0
    ...
    def display_bookmark(self, key, pos):
        ...
        try:
            ...
            if self.use_covers:
                ...
                if image:
                    try:
                        pix = icons.MANAGER.pixbuf_from_data(
                            image, size=(16, 16))
                    except GLib.GError:
        ...
        # closure magic (workaround for factories not having access to item)
        ...
        def factory(menu_, parent, context):
            ...
            if menu_ is self.menu:
                ...
                menu_item.connect(
                    'activate', self.delete_bookmark, (counter, key, pos))

            return menu_item

        item = menu.MenuItem('bookmark{0}'.format(
            self.counter), factory, ['sep'])
        self.menu.add_item(item)
