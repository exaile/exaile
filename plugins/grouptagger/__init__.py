...
class GroupTaggerPlugin(object):
    ...
    def on_gui_loaded(self):
        ...
        self.panel.tagger.view.connect(
            'category-changed', self.on_category_change)
        self.panel.tagger.view.connect(
            'category-edited', self.on_category_edited)
        ...
        event.add_ui_callback(self.on_playback_track_start,
                              'playback_track_start')
        event.add_ui_callback(
            self.on_playlist_cursor_changed, 'playlist_cursor_changed')
        event.add_ui_callback(self.on_plugin_options_set,
    ...
    def disable(self, exaile):
        ...
        event.remove_callback(
            self.on_playlist_cursor_changed, 'playlist_cursor_changed')
        event.remove_callback(self.on_plugin_options_set,
                              'plugin_grouptagger_option_set')
    ...
    def on_playlist_context_select_all_menu(self, menu, display_name, playlist_view, context, exaile):
        ...
        if len(groups) > 0:
            ...
            dialogs.error(
                None, _('No categorization tags found in selected tracks'))
