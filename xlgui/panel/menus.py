...
def __create_track_panel_menus():
    ...
    items.append(menuitems.ReplaceCurrentMenuItem(
        'replace', after=[items[-1].name]))
    ...
    items.append(menuitems.PropertiesMenuItem(
        'properties', after=[items[-1].name]))
...
def __create_collection_panel_context_menu():
    ...
    items.append(menuitems.OpenDirectoryMenuItem(
        'open-directory', after=[items[-1].name]))
    items.append(menuitems.TrashMenuItem(
        'trash-tracks', after=[items[-1].name], delete_tracks_func=collection_delete_tracks_func))
...
def __create_playlist_panel_playlist_menus():
    ...
    items.append(menuitems.RenamePlaylistMenuItem(
        'rename-playlist', after=[items[-1].name]))
    items.append(menuitems.EditPlaylistMenuItem(
        'edit-playlist', after=[items[-1].name]))
    items.append(menuitems.ExportPlaylistMenuItem(
        'export-playlist', after=[items[-1].name]))
    items.append(menuitems.ExportPlaylistFilesMenuItem(
        'export-files', after=[items[-1].name]))

    items.append(menu.simple_separator('pp-sep', after=[items[-1].name]))

    items.append(menuitems.DeletePlaylistMenuItem(
        'delete-playlist', after=[items[-1].name]))
...
class PlaylistsPanelPlaylistMenu(menu.MultiProviderMenu):
    ...
    def __init__(self, parent):
        menu.MultiProviderMenu.__init__(self,
                                        ['playlist-panel-menu', 'track-panel-menu',
    ...
    def get_context(self):
        context = common.LazyDict(self._parent)
        context[
            'selected-playlist'] = lambda name, parent: parent.tree.get_selected_page(raw=True)
        context[
            'selected-tracks'] = lambda name, parent: parent.tree.get_selected_tracks()
        context[
            'selection-empty'] = lambda name, parent: parent.tree.get_selection_empty()
...
class RadioPanelPlaylistMenu(menu.MultiProviderMenu):
    ...
    def __init__(self, parent):
        menu.MultiProviderMenu.__init__(self,
                                        ['radio-panel-menu', 'track-panel-menu',
                                            'playlist-panel-context-menu'],
                                        parent)
