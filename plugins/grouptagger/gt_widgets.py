...
class GTShowTracksMenuItem(menu.MenuItem):
    ...
    def factory(self, menu, parent, context):
        ...
        menuitem.connect(
            'activate', lambda *e: gt_common.create_all_search_playlist(context['groups'], parent.exaile))
        return menuitem
...
class GroupTaggerView(Gtk.TreeView):
    ...
    def __init__(self, exaile, model=None, editable=False):
        ...
        self._row_expanded_id = self.connect(
            'row-expanded', self.on_row_expanded)
        self._row_collapsed_id = self.connect(
            'row-collapsed', self.on_row_collapsed)
        ...
        self.append_column(Gtk.TreeViewColumn(
            _('Tag'), self.text_column, text=1, weight=3))
        ...
        item = smi('selcust', ['sel'], _('Show tracks with selected (custom)'),
                   callback=lambda w, n, p, c: gt_common.create_custom_search_playlist(c[
                                                                                       'groups'], exaile),
    ...
    def get_context(self):
        ...
        context[
            'selected-rows'] = lambda name, parent: parent.get_selection().get_selected_rows()
        context['groups'] = lambda name, parent: parent.get_selected_groups(context[
                                                                            'selected-rows'])
        ...
        if self.get_model() and not model.is_category(path):
            ...
            if category is not None:
                self.emit('category-changed',
                          category_change.updated, category)
                self.expand_to_path(path)

    def on_row_collapsed(self, widget, iter, path):
        self.emit('category-changed', category_change.collapsed,
    ...
    def on_row_expanded(self, widget, iter, path):
        self.emit('category-changed', category_change.expanded,
                  self.get_model().get_category(path))
    ...
    def on_menu_add_group(self, widget, name, parent, context):
        ...
        input = dialogs.TextEntryDialog(
            _('New tag value?'), _('Enter new tag value'))
        ...
        if input.run() == Gtk.ResponseType.OK:
            ...
            if category != "":
                ...
                if model.add_category(category):
                    self.emit('category-changed',
                              category_change.added, category)
...
class GroupTaggerTreeStore(Gtk.TreeStore, Gtk.TreeDragSource, Gtk.TreeDragDest):
    ...
    def add_group(self, group, category=uncategorized, selected=True):
        ...
        for row in self:
            if row[1] == category:
                ...
                self.append(row.iter, [selected, group,
    ...
    def delete_selected_categories(self, paths):
        ...
        categories = {}
        iters = [self.get_iter(path)
                 for path in paths if self[path].parent is None]
    ...
    def delete_selected_groups(self, paths):
        ...
        groups = []
        iters = [self.get_iter(path) for path in paths if self[
    ...
    def load(self, group_categories):
        ...
        for category, (expanded, groups) in group_categories.iteritems():
            cat = self.append(
                None, [expanded, category, False, Pango.Weight.BOLD])
...
class GroupTaggerWidget(Gtk.VBox):
    ...
    def __init__(self, exaile):
        ...
        self.view = GroupTaggerView(
            exaile, GroupTaggerTreeStore(), editable=True)
    ...
    def on_add_tag_click(self, widget):
        self.view.on_menu_add_group(
            self.view, None, None, self.view.get_context())
    ...
    def add_groups(self, groups):
        ...
        for group in groups:
            added = self.store.add_group(
                group, uncategorized, selected=False) or added

        self.view.set_model(self.store)
        self.view.sync_expanded()

        if added:
            self.view.emit('category-changed',
                           category_change.updated, uncategorized)
...
class AllTagsListView(Gtk.TreeView):
    ...
    def __init__(self, model=None):
        ...
        # Setup the second column
        self.append_column(Gtk.TreeViewColumn(
            _('Group'), Gtk.CellRendererText(), text=1))
...
class GroupTaggerQueryDialog(Gtk.Dialog):
    ...
    def __init__(self, groups):
        ...
        self.choices = [_('Must have this tag [AND]'), _('May have this tag [OR]'), _(
            'Must not have this tag [NOT]'), _('Ignored')]
    ...
    def get_search_params(self):
        ...
        if len(and_p[0]):
            ...
            and_p[1] = ' '.join(['%s~"\\b%s\\b"' % (tagname, re.escape(
                group.replace(' ', '_'))) for group in and_p[0]])
        ...
        if len(not_p[0]):
            ...
            not_p[1] = ' ! %s~"%s"' % (tagname, '|'.join(
                ['\\b' + re.escape(group.replace(' ', '_')) + '\\b' for group in not_p[0]]))
        ...
        if len(or_p[0]):
            ...
            or_p[1] = ' %s~"%s"' % (tagname, '|'.join(
                ['\\b' + re.escape(group.replace(' ', '_')) + '\\b' for group in or_p[0]]))
