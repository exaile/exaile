...
class GSTEqualizer(ElementBin):
	...
	def _on_setting_change(self, name, object, data):
		...
		if data == "plugin/equalizer/pre":
			if settings.get_option("plugin/equalizer/enabled") == True:		
				self.preamp.set_property("volume", self.dB_to_percent(settings.get_option("plugin/equalizer/pre")))
			else:
...
class EqualizerPlugin:
	...
	def __init__(self, exaile):
		...
		self.presets = gtk.ListStore(str, float, float, float, float, float, float, float, float, float, float, float)
		self.load_presets()
	...
	def adjust_band(self, widget, data):
		...
		"""
		if widget.get_value() != settings.get_option("plugin/equalizer/band%s" %widget.get_name()[-1]):
	...
	def add_preset(self, widget):
		...
		new_preset = []
		new_preset.append(self.ui.get_object("equalizer/combo-presets").get_child().get_text())
	...
	def preset_changed(self, widget):
		...
		if i > 0:
			...
			settings.set_option("plugin/equalizer/pre", d.get_value( d.get_iter(i), 1))
			self.ui.get_object("equalizer/pre").set_value( d.get_value( d.get_iter(i), 1) )

			for band in range(10):
				settings.set_option("plugin/equalizer/band%s"%band, d.get_value( d.get_iter(i), band+2))
	...
	def load_presets(self):
		...
		if os.path.exists(self.presets_path):
			...
			self.presets.append(["Dance", 0, 9.6, 7.2, 2.4, 0, 0, -5.6, -7.2, -7.2, 0, 0])
			self.presets.append(["Full Bass", 0, -8, 9.6, 9.6, 5.6, 1.6, -4, -8, -10.4, -11.2, -11.2])
			self.presets.append(["Full Bass and Treble", 0, 7.2, 5.6, 0, -7.2, -4.8, 1.6, 8, 11.2, 12, 12])
			self.presets.append(["Full Treble", 0, -9.6, -9.6, -9.6, -4, 2.4, 11.2, 16, 16, 16, 16.8])
			self.presets.append(["Laptop Speakers and Headphones", 0, 4.8, 11.2, 5.6, -3.2, -2.4, 1.6, 4.8, 9.6, 11.9, 11.9])
			self.presets.append(["Large Hall", 0, 10.4, 10.4, 5.6, 5.6, 0, -4.8, -4.8, -4.8, 0, 0])
			self.presets.append(["Live", 0, -4.8, 0, 4, 5.6, 5.6, 5.6, 4, 2.4, 2.4, 2.4])
			self.presets.append(["Party", 0, 7.2, 7.2, 0, 0, 0, 0, 0, 0, 7.2, 7.2])
			self.presets.append(["Pop", 0, -1.6, 4.8, 7.2, 8, 5.6, 0, -2.4, -2.4, -1.6, -1.6])
			self.presets.append(["Reggae", 0, 0, 0, 0, -5.6, 0, 6.4, 6.4, 0, 0, 0])
			self.presets.append(["Rock", 0, 8, 4.8, -5.6, -8, -3.2, 4, 8.8, 11.2, 11.2, 11.2])
			self.presets.append(["Ska", 0, -2.4, -4.8, -4, 0, 4, 5.6, 8.8, 9.6, 11.2, 9.6])
