...
class PlayQueue(playlist.Playlist):
    ...
    def __init__(self, player, name, location=None):
        ...
        self._on_option_set(
            None, settings, self.__opt_disable_new_track_when_playing)
    ...
    def _on_option_set(self, evtype, settings, option):
        if option == self.__opt_remove_item_when_played:
            ...
            self.__disable_new_track_when_playing = settings.get_option(
                option, False)
    ...
    # Internal value indicating whether the internal queue has tracks left to
    # play
    ...
    def _do_restore_player_state(self, state):
        ...
        if state['state'] in ['playing', 'paused']:
            ...
            paused = state['state'] == 'paused' or \
                settings.get_option("%s/resume_paused" %
                                    self.player._name, False)

            self.player.play(self.get_current(),
                             start_at=start_at, paused=paused)
