...
class AnalyzerDialog(object):
    ...
    def __build_playlist_list(self, selected_playlist):
        ...
        for name in sorted(manager.list_playlists()):
            ...
            if selected_playlist and selected_playlist.name == name:
                ...
                self.playlists_list.scroll_to_cell(
                    self.playlist_store.get_path(i))
    ...
    def __build_tag_table(self):
        ...
        for i in xrange(0, tmpl['maxtags']):
            ...
            self.tags_table.attach(label, 0, 1, i, i + 1,
                                   xoptions=0, yoptions=0)
            self.tags_table.attach(combo, 1, 2, i, i + 1,
                                   xoptions=0, yoptions=0)
            self.tags_table.attach(spin, 2, 3, i, i + 1,
                                   xoptions=0, yoptions=0)
        ...
        for i, (cb, sp) in enumerate(self.__tag_widgets):
            guiutil.persist_selection(
                cb, 2, 'plugin/playlistanalyzer/tag%s' % i)
    ...
    def on_generate_clicked(self, widget):
        ...
        if len(playlists) == 1:
            ...
            output_fname = '%s%sanalysis.html' % (
                pname, ' ' if ' ' in pname else '-')
