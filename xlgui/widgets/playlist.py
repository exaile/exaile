...
def __create_playlist_tab_context_menu():
    ...
    items.append(menuitems.ExportPlaylistMenuItem(
        'export', ['tab-close-sep'], _get_pl_func))
    items.append(menuitems.ExportPlaylistFilesMenuItem(
        'export-files', ['export'], _get_pl_func))
...
class PlaylistContextMenu(menu.ProviderMenu):
    ...
    def get_context(self):
        ...
        context[
            'selection-empty'] = lambda name, parent: parent.get_selection_count() == 0
...
def __create_playlist_context_menu():
    ...
    items.append(smi('playlist-menu', [items[-1].name], _('Playlist'),
                     submenu=menu.ProviderMenu(
                         'playlist-tab-context-menu', None),
...
class PlaylistPage(PlaylistPageBase):
    ...
    def __init__(self, playlist, player):
        ...
        self.on_mode_changed(
            None, None, self.playlist.repeat_mode, self.repeat_button)
        self.on_mode_changed(
        ...
        self.on_option_set('gui_option_set', settings,
                           'gui/playlist_utilities_bar_visible')
        self.view.connect('button-press-event',
                          self.on_view_button_press_event)
        ...
        if loading:
            if self.loading is None and self.loading_timer is None:
                self.loading_timer = GLib.timeout_add(
                    500, self.on_data_loading_timer)
        else:
...
class PlaylistView(AutoScrollTreeView, providers.ProviderHandler):
    ...
    def __init__(self, playlist, player):
        ...
        # MASSIVE speedup - don't disable this!
    ...
    def filter_tracks(self, filter_string):
        ...
        if filter_string is None:
            ...
            keyword_tags = set(playlist_columns.DEFAULT_COLUMNS +
                               [c.name for c in self.get_columns()])
            self._filter_matcher = trax.TracksMatcher(filter_string,
                                                      case_sensitive=False,
                                                      keyword_tags=keyword_tags)
            logger.debug("Filtering playlist '%s' by '%s'." %
                         (self.playlist.name, filter_string))
            self.modelfilter.refilter()
            logger.debug("Filtering playlist '%s' by '%s' completed." %
                         (self.playlist.name, filter_string))
    ...
    def get_selected_items(self):
        ...
        try:
            if isinstance(model, Gtk.TreeModelFilter):
                tracks = [(model.convert_path_to_child_path(path)[0], model.get_value(
                    model.get_iter(path), 0)) for path in paths]
            else:
                tracks = [(path[0], model.get_value(model.get_iter(path), 0))
                          for path in paths]
    ...
    def on_column_clicked(self, column):
        ...
        self.playlist.sort(
            [column.name] + list(common.BASE_SORT_TAGS), reverse=reverse)
    ...
    def scroll_to_current(self):
        ...
        if position >= 0:
            ...
            if hasattr(model, 'convert_child_path_to_path'):
                path = model.convert_child_path_to_path(
                    Gtk.TreePath((position,)))
    ...
    def on_drag_motion(self, widget, context, x, y, etime):
        ...
        target = self.drag_dest_find_target(
            context, self.drag_dest_get_target_list()).name()
...
class PlaylistModel(Gtk.ListStore):
    ...
    def __init__(self, playlist, columns, player):
        ...
        self.coltypes = [object, GdkPixbuf.Pixbuf] + \
            [providers.get_provider(
                'playlist-columns', c).datatype for c in columns]
        ...
        self._setup_icons()
        self.on_tracks_added(None, self.playlist, list(
            enumerate(self.playlist)))  # populate the list
    ...
    def _setup_icons(self):
        ...
        if font is not None:
            # get default font
            default = float(
                Gtk.Widget.get_default_style().font_desc.get_size())
            ...
            self.pause_stop_pixbuf = self.pause_stop_pixbuf.scale_simple(
                s, s, t)
    ...
    def icon_for_row(self, row):
        # TODO: we really need some sort of global way to say "is this
        # playlist/pos the current one?
    ...
    def _on_track_tags_changed(self):
        ...
        for row in self:
            ...
            if track is not None:
                track_data = [providers.get_provider(
                    'playlist-columns', name).formatter.format(track) for name in self.columns]
    ...
    def _load_data(self, tracks):
        ...
        # get column types
        coltypes = [self.get_column_type(i)
                    for i in xrange(self.get_n_columns())]
        formatters = [providers.get_provider(
            'playlist-columns', name).formatter.format for name in self.columns]
    ...
    def _load_data_fn(self, coltypes, formatters, tracks):
        ...
        for position, track in tracks:
            track_data = [track, self.icon_for_row(
                position).pixbuf] + [formatter(track) for formatter in formatters]
            render_data.append(
                (position, [Value(typ, val) for typ, val in izip(coltypes, track_data)]))
