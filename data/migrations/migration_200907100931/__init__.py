...
_SETTINGS_MAP = (
    ...
    (str,   'lastfm',   'pass',
     'plugin/ascrobbler/password', '_set_lastfm_password'),
    ...
    (str,   'ui',       'tab_placement',
     'gui/tab_placement', '_set_tab_placement'),
)
...
def migration_needed():
    ...
    for file in ('~/.exaile/music.db', '~/.exaile/settings.ini'):
        if not os.path.exists(os.path.expanduser(file)):
            logger.debug(
                "%s did not exist, old exaile version not detected" % file)
            return False
...
def _migrate_old_tracks(oldsettings, db, ntdb):
    ...
    for oldtrack in oldtracks:
        # we shouldn't be checking os.path.isfile() here, since if it is a
        # radio link, it will not be migrated
        ...
        if oldtrack._rating:  # filter '' and 0
            ...
            newtrack.set_tag_raw('__rating', float(
                (100.0 * oldtrack._rating) / rating_steps))
        ...
        # Apparently, there is a bug in exaile 0.2.xx that dumps the time as
        # hh:mm:YYYY, rather than hh:mm:ss. This is a workaround, that takes
        # the seconds == 0, since this information is lost b/c of the bug
        temp_time = oldtrack.time_added

        try:
            newtrack.set_tag_raw('__date_added', time.mktime(
                time.strptime(temp_time[0:len(temp_time) - 5], '%Y-%m-%d %H:%M')))
        except ValueError:
            try:
                newtrack.set_tag_raw('__date_added', time.mktime(
                    time.strptime(temp_time[0:len(temp_time) - 3], '%Y-%m-%d %H:%M')))
            except ValueError:
                pass
