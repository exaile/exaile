...
def initialize():
	...
	try:
		...
		raise plugins.PluginInitException("Sound Juicer was not found in your $PATH. "
			"Disabling the Sound Juicer plugin.")
		return False

	menu = APP.xml.get_widget('file_menu_menu')
	MENU_ITEM = gtk.ImageMenuItem('Import Disc')
	MENU_ITEM.label = gtk.Label('Import Disc')
	MENU_ITEM.connect('activate',launch_sound_juicer)
	image = gtk.Image()
	image.set_from_stock('gtk-cdrom', gtk.ICON_SIZE_MENU)
	MENU_ITEM.set_image(image)
	menu.append(MENU_ITEM)
	menu.reorder_child(MENU_ITEM,4)
	MENU_ITEM.show_all()
	return True
