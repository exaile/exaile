...
class FormatConverter(object):
    ...
    def get_track_import_path(self, playlist_path, track_path):
        ...
        track_uri = urlparse.urljoin(
            playlist_uri, track_path.replace('\\', '/'))
        ...
        if track_uri.startswith('file:///') and \
            ...
            if not playlist_uri.startswith('file:///'):
                logging.debug(
                    'Track does not seem to exist, using original path')
            else:
                logging.debug(
    ...
    def get_track_export_path(self, playlist_path, track_path, options):
        ...
        if options is not None and options.relative:
            ...
            export_path = playlist_file.get_uri(
            )[:-len(playlist_file.get_basename())]
    ...
    def export_to_file(self, playlist, path, options=None):
        ...
        with GioFileOutputStream(Gio.File.new_for_uri(path)) as stream:
            ...
            for track in playlist:
                title = [track.get_tag_display(
                    'title', join=True), track.get_tag_display('artist', join=True)]
                length = int(round(float(track.get_tag_raw('__length') or -1)))

                track_path = track.get_loc_for_io()
                track_path = self.get_track_export_path(
                    path, track_path, options)
...
class PLSConverter(FormatConverter):
    ...
    def export_to_file(self, playlist, path, options=None):
        ...
        for index, track in enumerate(playlist):
            ...
            pls_playlist.set('playlist', 'Title%d' %
                             position, ' - '.join(title))
...
class Playlist(object):
    ...
    def __init__(self, name, initial_tracks=[]):
        ...
        for track in initial_tracks:
            if not isinstance(track, trax.Track):
                raise ValueError(
                    "Need trax.Track object, got %r" % type(track))
    ...
    def set_current_position(self, position):
        ...
        if position != -1:
            ...
            self.__tracks.set_meta_key(
                position, "playlist_current_position", True)
        self.__current_position = position
        if oldposition != -1:
            try:
                self.__tracks.del_meta_key(
                    oldposition, "playlist_current_position")
            except KeyError:
                pass
        self.__dirty = True
    ...
    def set_spat_position(self, position):
        ...
        if oldposition != -1:
            try:
                self.__tracks.del_meta_key(
                    oldposition, "playlist_spat_position")
            except KeyError:
                pass
        self.__dirty = True
        event.log_event("playlist_spat_position_changed",
                        self, (position, oldposition))
    ...
    def __get_next(self, current_position):
        ...
        if shuffle_mode != 'disabled':
            ...
            next_index, next = self.__next_random_track(
                current_position, shuffle_mode)
            if next is not None:
    ...
    def randomize(self, positions=None):
        ...
        if positions:
            ...
            if len(positions) == 2:
                ...
                shuffle_tracks = [t for i, t in enumerate(
                    tracks) if i in positions]
        ...
        for loc in locs:
            ...
            if not track.is_local() and meta is not None:
                ...
                for k, v in meta.iteritems():
                    track.set_tag_raw(k, v[0].decode(
                        'utf-8'), notify_changed=False)
        ...
        if isinstance(i, slice):
            for x in value:
                if not isinstance(x, trax.Track):
                    raise ValueError(
                        "Need trax.Track object, got %r" % type(x))
            ...
            if step != 1:
                if len(value) != len(oldtracks):
                    raise ValueError(
                        "Extended slice assignment must match sizes.")
            ...
            if not isinstance(value, trax.Track):
                raise ValueError(
                    "Need trax.Track object, got %r" % type(value))
