...
class ExaileScrobbler(object):
    ...
    def get_options(self, type, sm, option):
        ...
        if option == 'plugin/ascrobbler/menu_check':
            self.use_menu = settings.get_option(
                'plugin/ascrobbler/menu_check', False)
            if self.use_menu:
                self.setup_menu()
            else:
                self.remove_menu()
    ...
    def stop(self):
        ...
        if self.connected:
            event.remove_callback(
                self.on_play, 'playback_track_start', player.PLAYER)
            event.remove_callback(
                self.on_stop, 'playback_track_end', player.PLAYER)
            self.connected = False
            self.save_cache()
        ...
        if scrobbler.SESSION_ID and track and time_started and time_played:
            try:
                scrobbler.submit(
                    ...
                    int(track.get_tag_raw('__length')),
                    track.get_tag_raw('album', join=True),
                    track.split_numerical(
                        track.get_tag_raw('tracknumber'))[0] or 0,
                    autoflush=True,
                )
            except:
