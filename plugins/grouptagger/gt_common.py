...
def migrate_settings():
    ...
    if settings.get_option(migrated_option, False):
        ...
        default_groups = settings.get_option(
            'plugin/grouptagger/default_groups', None)
...
def set_track_groups(track, groups):
    ...
    if not track.write_tags():
        dialogs.error(None, "Error writing tags to %s" %
...
def _create_search_playlist(name, search_string, exaile):
    ...
    tracks = [x.track for x in search.search_tracks_from_string(
        exaile.collection, search_string)]
...
def create_all_search_playlist(groups, exaile):
    ...
    search_string = ' '.join(['%s~"\\b%s\\b"' % (
        tagname, re.escape(group.replace(' ', '_'))) for group in groups])
