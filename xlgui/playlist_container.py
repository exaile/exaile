...
class NewPlaylistNotebookAction(NotebookAction, Gtk.Button):
    ...
    def on_drag_data_received(self, widget, context, x, y, selection, info, time):
        ...
        tab.page.view.emit('drag-data-received', context,
...
class PlaylistNotebook(SmartNotebook):
    ...
    def __init__(self, manager_name, player, hotkey):
        ...
        self.tab_history = []
        # to get unique (reverse-ordered) item names
        self.history_counter = 90000
        ...
        item = menu.MenuItem('%s-tab-history' %
                             manager_name, factory, ['tab-close'])
        ...
        self.accelerator = Accelerator(
            hotkey, lambda *x: self.restore_closed_tab(0))
    ...
    def create_new_playlist(self):
        ...
        for n in range(self.get_n_pages()):
            ...
            if name_parts == default_name_parts:
                ...
                number = name[len(name_parts[0]):len(
                    name) - len(name_parts[1])]
    ...
    def save_closed_tab(self, playlist):
        ...
        def factory(menu_, parent, context):
            ...
            item.set_image(Gtk.Image.new_from_icon_name(
                'music-library', Gtk.IconSize.MENU))
...
class PlaylistContainer(Gtk.HBox):
    ...
    def __init__(self, manager_name, player):
        ...
        self.notebooks = []
        self.notebooks.append(PlaylistNotebook(
            manager_name, player, '<Control><Shift>t'))
        self.notebooks.append(PlaylistNotebook(
            manager_name + '2', player, '<Control><Alt>t'))
    ...
    def show_queue(self, switch=True):
        ...
        if self.queuepage.tab.notebook is None:
            ...
            self.notebooks[n].add_tab(
                self.queuetab, self.queuepage, position=0)
        if switch:
