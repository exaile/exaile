...
class GtImporter(Gtk.Window):
    ...
    def __init__(self, exaile, uris):
        ...
        self.manager.add_monitor(self.rescan_thread, _(
            "Importing tracks"), Gtk.STOCK_REFRESH)
    ...
    def _on_rescan_done(self):
        ...
        self.import_thread = TrackImportThread(
            self.collection, self.exaile.collection)
        self.import_thread.connect('done', self._on_import_done)
        self.manager.add_monitor(self.import_thread, _(
            "Importing groups"), Gtk.STOCK_JUMP_TO)
    ...
    def _on_import_done(self, thread):
        ...
        if len(track_data) == 0:
            ...
            locations = ';'.join([l.get_location()
                                  for l in self.collection.get_libraries()])

            dialogs.info(self.exaile.gui.main.window,
        ...
        for old_group_str, new_group_str, matched_track, newgroups in track_data:
            self.tags_model.append(
                (True, str(matched_track), old_group_str, new_group_str, matched_track, newgroups))
    ...
    def on_ok_button_clicked(self, widget):
        ...
        self.update_thread = TrackUpdateThread(
            data, self.radio_replace.get_active())
        self.update_thread.connect('done', self._on_update_done)

        self.manager.add_monitor(self.update_thread, _(
            "Updating groups"), Gtk.STOCK_CONVERT)
...
class TrackImportThread(ProgressThread):
    ...
    def run(self):
        ...
        fields = ['__length', 'artist', 'album',
                  'title', 'genre', 'tracknumber']
        ...
        for i, track in enumerate(self.import_collection):
            ...
            matchers = map(lambda t: TracksMatcher(
                track.get_tag_search(t)), fields)
            matched_tracks = [r.track for r in search_tracks(
                self.user_collection, matchers)]
        ...
        logger.info("Match information: %s exact dups, %s no change, %s differing tracks" % (
