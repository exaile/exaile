	...
	if APP.settings.get_boolean(PLAYING, False):
		if APP.tracks:
			...
			if track < len(songs):
				...
				APP.current_track.seek(APP.settings.get_float(PROGRESS,0.1))
			elif len(songs) == 0:
				return True
...
def save_state(sender):
	...
	if track:
		APP.settings[PLAYING] = track.is_playing()
		if APP.tracks:
			songs = APP.tracks.get_songs()
			if songs:
				APP.settings[TRACK] = songs.index(track)
				APP.settings[PROGRESS] = track.current_position() * track.duration /\
						100.0
	else:
