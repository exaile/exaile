...
def _init(prefsdialog, builder):
    ...
    # note that we get a new builder everytime the prefs dialog is closed and
    # re-opened
    MY_BUILDER = builder
...
#        self.view = None
    ...
    def enable_cb(self, cell, path):
        ...
        logger.debug('Alarm {0} {1}abled.'.format(
            self.model[path][1], 'dis' if active else 'en'))
...
#            return
        ...
        # NOTE: reloading the plugin from prefs page (DEBUG MODE) breaks this
        # anyway
    ...
    def days_changed(self, cr, path, days):
        ...
        self.model[path][4] = ','.join(
            [days_str[i] for i in range(0, 7) if days[i]])
    ...
    def text_edited(self, cr, path, new_text, idx):
        ...
        if idx == 1:  # Name edit
            ...
            try:
                ...
                # ensure consistent 0-padding
                new_text = time.strftime('%H:%M', t)
            except ValueError:
    ...
    def add_alarm(self, alarm):
        ...
        # update display
        days_str = ['Su', 'M', 'Tu', 'W', 'Th', 'F', 'Sa']
        day_disp = ','.join([days_str[i]
                             for i in range(0, 7) if alarm.days[i]])
        self.model.append(
            [alarm.active, alarm.name, alarm.time, alarm.days, day_disp])
                ...
                try:
                    ...
                    try:
                        ...
                        for line in raw.strip().split('\n'):
                            ...
                            logger.debug(
                                'loaded alarm {0} ({1}) from file.'.format(a.name, a.time))
                            self.add_alarm(a)
                    ...
                    for a in alist:
                        alarm = Alarm(dict=a)
                        logger.debug('loaded alarm {0} ({1}) from file.'.format(
...
def fade_in(main, exaile):
    ...
    # pull settings
    temp_volume = settings.get_option(
        'plugin/multialarmclock/fade_min_volume') / 100.
    fade_max_volume = settings.get_option(
        'plugin/multialarmclock/fade_max_volume') / 100.
    fade_inc = settings.get_option(
        'plugin/multialarmclock/fade_increment') / 100.
    time_per_inc = settings.get_option(
        'plugin/multialarmclock/fade_time') / ((fade_max_volume - temp_volume) / fade_inc)
...
def check_alarms(main, exaile):
    ...
    alist = [Alarm(active=row[0], name=row[1], time=row[2], days=row[3])
