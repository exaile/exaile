...
class UDisksPropertyWrapper(object):
    ...
    # return self.obj.connect_to_signal('PropertiesChanged', fn,
    # self.iface_type)
...
class UDisksBase(providers.ProviderHandler):
    ...
    def _add_device(self, path, obj=None):
        ...
        try:
            ...
            logger.exception(
                "%s: Failed autoconnecting device %s", self.name, str(device))
        else:
    ...
    def _udisks_device_added(self, *args):
        ...
        if self._addremove():
            try:
                ...
                self._state = 'listening'
                logger.debug(
                    "%s: state = listening (_device_added)", self.name)
    ...
    def _udisks_device_removed(self, *args):
        ...
        if self._addremove():
            try:
                ...
                self._state = 'listening'
                logger.debug(
                    "%s: state = listening (_device_removed)", self.name)
    ...
    def on_provider_added(self, provider):
        if self._addremove():
            try:
                ...
                self._state = 'listening'
                logger.debug(
                    "%s: state = listening (_provider_added)", self.name)
    ...
    def on_provider_removed(self, provider):
        if self._addremove():
            try:
                ...
                self._state = 'listening'
                logger.debug(
                    "%s: state = listening (_provider_removed)", self.name)
        ...
        try:
            ...
            for p in self.get_providers():
                try:
                    ...
                    logger.exception(
                        "Failed to load HAL devices for %s", p.name)
    ...
    def get_handler(self, udi):
        ...
        try:
            ...
            if not e.get_dbus_name() == "org.freedesktop.Hal.NoSuchProperty":
                logger.exception(
                    "info.capabilities property not set for %s", udi)
            return None
        handlers = []
