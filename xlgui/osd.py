...
class OSDWindow(object):
    ...
    def setup_osd(self):
        ...
        self.color = gtk.gdk.color_parse(
                SETTINGSMANAGER.get_option('osd/bg_color', '#567ea2'))
        ...
        self.cover_widget.set_image_size(
            SETTINGSMANAGER.get_option('osd/h', 95) - 8,
            SETTINGSMANAGER.get_option('osd/h', 95) - 8)
        ...
        text = "<span font_desc='%s' foreground='%s'>%s</span>" % \
            (SETTINGSMANAGER.get_option('osd/text_font', 'Sans 11'),
            SETTINGSMANAGER.get_option('osd/text_color', '#ffffff'),
            SETTINGSMANAGER.get_option('osd/display_text', 
                "<b>{title}</b>\n{artist}\non {album} - {length}"))
        self.title.set_markup(text)
        self.text = text

        self.window.set_size_request(
            SETTINGSMANAGER.get_option('osd/w', 400), 
            SETTINGSMANAGER.get_option('osd/h', 95))
        self.window.move(
            SETTINGSMANAGER.get_option('osd/x', 0), 
            SETTINGSMANAGER.get_option('osd/y', 0))
    ...
    def expose_callback(self, widget, event):
        ...
        opacity = int(SETTINGSMANAGER.get_option('osd/opacity', 75))
    ...
    def stop_dragging(self, widget, event):
        ...
        for key, val in (('osd/x', int(x)), 
                            ...
                            ('osd/h', int(h)),
                            ('osd/w', int(w))):
            SETTINGSMANAGER.set_option(key, val)
    ...
    def hide_progress(self, *args):
        if not SETTINGSMANAGER.get_option('osd/show_progress', True):
