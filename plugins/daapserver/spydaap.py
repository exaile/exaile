...
md_cache = spydaap.metadata.MetadataCache(
    os.path.join(spydaap.cache_dir, "media"), spydaap.parsers)
container_cache = spydaap.containers.ContainerCache(
    os.path.join(spydaap.cache_dir, "containers"), spydaap.container_list)
keep_running = True
...
def usage():
    ...
    sys.stderr.write(
        "  -f, --foreground        run in foreground, rather than daemonizing\n")
    sys.stderr.write("  -g, --group=groupname   specify group to run as\n")
    sys.stderr.write("  -h, --help              print this help\n")
    sys.stderr.write(
        "  -l, --logfile=file      use .log file (default is ./spydaap.log\n")
    sys.stderr.write(
        "  -p, --pidfile=file      use .pid file (default is ./spydaap.pid\n")
...
def main():
    ...
    gid = os.getgid()
    try:
        opts, args = getopt.gnu_getopt(sys.argv[1:], "fg:hl:p:u:", [
                                       "foreground", "group=", "help", "logfile=", "pidfile=", "user="])
        for o, a in opts:
            if o in ("-h", "--help"):
                usage()
                sys.exit()
