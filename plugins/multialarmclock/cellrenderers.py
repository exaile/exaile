...
class CellRendererDays(Gtk.CellRendererText):
    ...
    def _create_view(self, treeview):
        ...
        self.view_window.set_transient_for(
            None)  # cancel the modality of dialog
    ...
    def do_start_editing(self, event, treeview, path, background_area, cell_area, flags):
        ...
        x = tree_x + min(cell_area.x, tree_w - my_w +
                         treeview.get_visible_rect().x)
        y = tree_y + min(cell_area.y, tree_h - my_h +
                         treeview.get_visible_rect().y)
        self.view_window.move(x, y)

        # save the path so we can return it in _done, and we aren't using
        # dialog so we can't block....
        self._path = path
