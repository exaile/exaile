...
class LVPanel():
    ...
    def done_getting_chapters(self, row):
        ...
        for chapter in self.books[row].chapters:
            self.rowlvl2 = self.treestore.append(
                iter, [chapter[0], self.chapter_icon])
    ...
    def __init__(self, exaile):
        ...
        self.abicon = GdkPixbuf.Pixbuf.new_from_file(
            self.librivoxdir + '/ebook.png')
    ...
    def gui_init(self, exaile):
        ...
        self.add_to_pl.set_image(Gtk.Image.new_from_stock(
            Gtk.STOCK_ADD, Gtk.IconSize.MENU))
        self.about_book = Gtk.ImageMenuItem.new_with_label("About the Book")
        self.about_book.set_image(Gtk.Image.new_from_stock(
    ...
    def generate_treestore(self, books):
        ...
        for book in books:
            self.rowlvl1 = self.treestore.append(
                None, [book.title, self.abicon])
            self.rowlvl2 = self.treestore.append(
        ...
        if len(path) == 1:  # book selected
            ...
            if book.loaded:  # chapters loaded
                ...
                if drop_info:
                    ...
                    self.get_chapters_and_drop(
                        book, current_playlist, PLpath, after)
