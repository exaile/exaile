...
class ExaileGstEngine(ExaileEngine):
    ...
    def _reconfigure_crossfader(self):
        ...
        if self.crossfade_enabled:
            ...
            if self.other_stream is None:
                ...
                self.other_stream.set_user_volume(
                    self.main_stream.get_user_volume())

            self.other_stream.reconfigure_fader(cf_duration, cf_duration)
            self.logger.info("Crossfade: enabled (%sms)",
    ...
    def _autoadvance_track(self, still_fading=False):
        ...
        if track:
            play_args = self.player.engine_autoadvance_notify_next(
                track) + (False, True)
...
class AudioStream(object):
    ...
    def play(self, track, start_at, paused, already_queued,
        ...
        self.fader.setup_track(track, fade_in_duration,
                               fade_out_duration, now=0)
    ...
    def on_message(self, bus, message):
        ...
        if message.type == Gst.MessageType.BUFFERING:
            ...
            if percent % 5 == 0:
                event.log_event('playback_buffering',
                                self.engine.player, percent)
            ...
            Gst.debug_bin_to_dot_file(
                self.playbin, Gst.DebugGraphDetails.ALL, self.name)
