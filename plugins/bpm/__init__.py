        ...
        if track and bpm:
            ...
            if result == Gtk.ResponseType.YES:
                ...
                if not track.write_tags():
                    dialogs.error(None, "Error writing BPM to %s" %
...
class BPMWidget(Gtk.Frame):
    ...
    def __init__(self, player, plugin):
        ...
        event.add_ui_callback(self.playback_track_start,
                              'playback_track_start', self.player)

        # get the main exaile window, and dock our window next to it if
        # possible
    ...
    def on_destroy(self, widget):
        ...
        event.remove_callback(self.playback_track_start,
                              'playback_track_start', self.player)
    ...
    def add_bpm_tap(self):
        ...
        if len(self.taps) > 1:
            self.bpm = str(
                int(round(((len(self.taps) - 1) * 60.0) / (self.taps[-1] - self.taps[0]))))
        else:
            self.bpm = None
