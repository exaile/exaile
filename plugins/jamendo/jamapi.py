...
class get_artist_list(threading.Thread):
    ...
    def run(self):
        url = "http://api.jamendo.com/get2/name+id/artist/json/?searchquery=%s&order=%s&n=%s" % (
...
class get_artist_list_by_genre(threading.Thread):
    ...
    def run(self):
        url = "http://api.jamendo.com/get2/name+id/artist/json/?tag_idstr=%s&order=%s&n=%s" % (
            self.search_term, self.order_by, self.num_results)
        results = get_json(url)
        artists = []
        for result in results:
...
class get_track_list(threading.Thread):
    ...
    def run(self):
        ...
        track_list = []
        for track in tracks:
            item = jamtree.Track(track['id'], track[
                                 'name'].strip(), track['stream'])
...
class get_albums(threading.Thread):
    ...
    def run(self):
        ...
        for albumresult in albumresults:
            item = jamtree.Album(
                albumresult['id'], albumresult['name'].strip())
