...
_corefoundation.CFStringGetCStringPtr.argtypes = [
...
def __setup_coreaudio():
    ...
    fns = [
        ('AudioObjectGetPropertyDataSize', c_uint32,
         (c_uint32, c_void_p, c_uint32, c_void_p, POINTER(c_uint32))),
        ('AudioObjectGetPropertyData', c_uint32, (c_uint32,
                                                  c_void_p, c_uint32, c_void_p, POINTER(c_uint32), c_void_p))
    ]
...
def get_devices():
    ...
    status = _coreaudio.AudioObjectGetPropertyDataSize(
        kAudioObjectSystemObject, ctypes.byref(propertyAddress), 0, None, dataSize)
    ...
    # get the data
    status = _coreaudio.AudioObjectGetPropertyData(kAudioObjectSystemObject, ctypes.byref(
        propertyAddress), 0, None, dataSize, audioDevices)
    if status != kAudioHardwareNoError:
        return
    ...
    for deviceId in audioDevices:
        ...
        status = _coreaudio.AudioObjectGetPropertyDataSize(
            deviceId, byref(propertyAddress), 0, None, dataSize)
        ...
        status = _coreaudio.AudioObjectGetPropertyData(deviceId, byref(
            propertyAddress), 0, None, dataSize, byref(buffer_list))
        ...
        status = _coreaudio.AudioObjectGetPropertyData(
            deviceId, byref(propertyAddress), 0, None, dataSize, byref(namep))
        if status != kAudioHardwareNoError:
            continue
